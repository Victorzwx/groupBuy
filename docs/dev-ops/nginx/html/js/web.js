        ...
        intervalId = setInterval(async () => {
            try {
                ...
                if (data.code === "0000") {
                    ...
                    // 设置用户cookie（30天有效期）
                    document.cookie = `userId=${data.data}; max-age=${30 * 24 * 60 * 60}; path=/`;
                    close();                   // 关闭弹窗
...
const ProductModule = (() => {
    ...
    const fetchData = async () => {
        try {
            ...
            userPayData = await response.json();

            updateUI(userPayData.data);  // 成功则更新界面
        } catch (error) {
    ...
    const PaymentModal = {
        show: (type,teamIdJoin) => {
            const modal = document.querySelector('.payment-mask');
            modal.style.display = 'flex';

            // 设置支付价格（确保元素选择正确）
            document.getElementById('userPayPrice').textContent = userPayData.data.goods.originalPrice;
            //在控制台打印userPayPrice的值
            //console.log('userPayPrice:', userPayData.data.goods.originalPrice);
            ...
            return new Promise((resolve) => {
                ...
                // 关闭按钮（新增事件绑定）
                document.querySelector('.close-btn-pay').onclick = () => closeHandler(false);

                // 取消按钮
                document.querySelector('.cancel-btn').onclick = () => closeHandler(false);

                // 确认支付按钮
                document.querySelector('.confirm-btn').onclick = async () => {

                    let userId = getCookie('userId') || "zwx"; // 简化代码
                    if (userId === "zwx") {
                        alert('请先登录');
                        return closeHandler(false); // 关闭弹窗
                    }

                    //
                    var productId = "9890001";
                    const activityId=100123;
                    ...
                    if(type=="立即参团"){
                        ...
                        var teamId = teamIdJoin;

                    }else {
                        var teamId=null;
                    }
                    //  console.log(teamId)
                    // console.log(teamIdJoin)
                    // console.log(userPayData.data.teamList)
                    // console.log(userPayData.data.teamList[0].teamId)
                    ...
                    // console.log('序列化后的请求体:', JSON.stringify(requestBody));
        ...
        hide: () => {
            //document.querySelector('.payment-mask').style.display = 'none';
            document.querySelector('.payment-mask').style.display = 'none';
        }
    };

// 复制沙箱账号功能
    document.getElementById('alipayAccount').addEventListener('click', () => {
        navigator.clipboard.writeText('qpatag6896@sandbox.com')
            .then(() => alert('账号已复制'))
    ...
    const handleButtonClick = async (btn) => {
        ...
        switch(btnType) {
            case '单独购买':
                //alert('正在进入单独购买流程...');
                await PaymentModal.show('单独购买',null);

                break;
            case '发起拼团':
                await PaymentModal.show('发起拼团',null);
                break;
            case '立即参团':
                var teamIdJoin=btn.dataset.teamId;
                // console.log(btn.dataset.teamId);
                await PaymentModal.show('立即参团',teamIdJoin);
                break;
        }
        return true;
    };
